...
module.exports = {
  async up(queryInterface) {
    const now = new Date();
    const codes = ['notifications:read', 'notifications:write'];
    ...
    const [roles] = await queryInterface.sequelize.query(
      `SELECT id FROM roles WHERE name IN ('system_admin','editor','content_manager')`,
    );
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE code LIKE 'notifications:%')`,
    );
    await queryInterface.sequelize.query(
      `DELETE FROM permissions WHERE code LIKE 'notifications:%'`,
    );
  },
};
